...
; Ensure compatibility stub exists for old backup versions
if !FileExist(A_ScriptDir "\BNH_QuickSMS.ahk") {
    try FileAppend("; BNH_QuickSMS.ahk - Compatibility stub`n; This file exists to prevent #Include errors in old backup versions.`n; All Quick SMS functionality is now built into the main script (BNH_Hotkey_V5_DEMO.ahk).`n", A_ScriptDir "\BNH_QuickSMS.ahk")
}
...
UpdateScript(newFilePath, newVersion) {
    try {
        ...
        ; Ensure compatibility stub exists so old backups won't crash with #Include error
        stubFile := A_ScriptDir "\BNH_QuickSMS.ahk"
        if !FileExist(stubFile) {
            try {
                FileAppend("; BNH_QuickSMS.ahk - Compatibility stub`n; This file exists to prevent #Include errors in old backup versions.`n; All Quick SMS functionality is now built into the main script (BNH_Hotkey_V5_DEMO.ahk).`n", stubFile)
            }
        }
...
:*:*ikkeledig::
{
    try {
        ...
        SendText("Vi har derfor foreløpig flyttet timen til første ledige tidspunkt.`r`n`r`n")
